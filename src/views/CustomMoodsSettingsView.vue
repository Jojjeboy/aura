  ...
  <div class="pb-24 space-y-8">
    <div class="px-6 space-y-6 pt-6">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-aura-text dark:text-aura-text-dark">{{ $t('settings_custom_moods') }}</h1>
      </div>
      ...
      <section class="space-y-4">
         <div class="bg-white dark:bg-aura-card-dark rounded-card shadow-soft p-1">
            <div v-if="settingsStore.customMoods.length === 0" class="p-6 text-center text-sm text-aura-muted italic">
               {{ $t('no_items') }}
            </div>
            <div v-else class="divide-y divide-slate-50 dark:divide-slate-800">
               <div
                  v-for="group in groupedCustomMoods"
                  :key="group.affectId"
                  class="p-4"
               >
                  ...
                  <div class="space-y-2">
                     ...
                     >
                        <div class="flex-1 min-w-0 pr-4">
                           <template v-if="editingMood === moodItem.mood && editingAffectId === moodItem.affectId">
                              <input
                           ...
                           <span v-else class="text-aura-text dark:text-aura-text-dark font-medium truncate block py-1">
                              {{ moodItem.mood }}
                           </span>
                        </div>
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
                           ...
                           >
                              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7h-12v13a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V7Z" />
                              </svg>
                           </button>
                        </div>
                     </div>
...
const handleRemoveCustomMood = async (mood: string, affectId: string) => {
    if (confirm(t('remove_lock_confirm_msg') || `Ta bort "${mood}"?`)) {
        await settingsStore.removeCustomMood(mood, affectId)
    }
}
</script>
